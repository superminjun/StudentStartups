      ...
      <div className="sticky top-0 min-h-screen overflow-hidden pt-[4.75rem]">
        ...
        <motion.div style={{ opacity }} className="mx-auto grid min-h-[calc(100svh-4.75rem)] w-full max-w-[90rem] items-center gap-8 px-5 py-8 sm:px-8 lg:grid-cols-[1.08fr_.92fr] lg:gap-16 lg:py-12">
          <motion.div style={{ y: titleY, scale: titleScale }} className="relative z-10 origin-left">
            <motion.p initial={{ opacity: 0, y: 14, filter: 'blur(7px)' }} animate={{ opacity: 1, y: 0, filter: 'blur(0px)' }} transition={motionSpring.reveal} className="flex items-center gap-3 text-[10px] font-semibold uppercase tracking-[.3em] text-black/45">
              <span className="h-px w-8 bg-[var(--ss-bronze)]" /> {t('hero.tagline')} · Est. 2024
            </motion.p>
            <h1 className="mt-7 max-w-[11.5ch] text-[clamp(3.25rem,7.5vw,8.25rem)] font-medium leading-[.84] tracking-[-.082em] text-[var(--ss-ink)]">
              {words.map((word, index) => (
                <span key={`${word}-${index}`} className="mr-[.2em] inline-block overflow-hidden pb-[.08em]">
                  <motion.span className="inline-block" initial={{ y: '112%', rotateX: 12, filter: 'blur(9px)' }} animate={{ y: 0, rotateX: 0, filter: 'blur(0px)' }} transition={{ ...motionSpring.depth, delay: .08 + index * STAGGER }}>{word}</motion.span>
                </span>
              ))}
            </h1>
    ...
    <article className="h-full w-screen shrink-0 px-5 sm:px-8">
      <div className="mx-auto grid h-full max-w-[90rem] items-center gap-8 pb-16 pt-24 md:grid-cols-[.78fr_1.22fr] md:gap-14 lg:gap-24">
        <motion.div style={{ y: copyY, opacity: copyOpacity }} className="relative z-10 self-end pb-2 md:self-center md:pb-0">
          <p className="text-[9px] font-semibold uppercase tracking-[.28em] text-white/38">{scene.no} / 04 · {scene.tag}</p>
          <p className="mt-4 text-[clamp(3.5rem,7vw,8rem)] font-medium leading-[.78] tracking-[-.085em] text-white">{scene.verb}.</p>
          <motion.div style={{ scaleX: ruleScale }} className="my-6 h-px origin-left bg-[var(--ss-bronze)]" />
          <h2 className="max-w-xl text-xl font-medium leading-tight tracking-[-.04em] text-white sm:text-3xl lg:text-4xl">{scene.title}</h2>
    ...
    <div className="bg-[var(--ss-canvas)]">
      ...
      <section className="px-5 py-24 sm:px-8 lg:py-36">
        <div className="mx-auto max-w-[90rem]">
          <ScrollReveal className="grid gap-8 lg:grid-cols-[.55fr_1.45fr] lg:items-end"><p className="text-[10px] font-semibold uppercase tracking-[.28em] text-black/42">{t('proof.kicker')} / 2024—26</p><h2 className="max-w-5xl text-[clamp(3rem,6vw,7rem)] font-medium leading-[.9] tracking-[-.072em] text-[var(--ss-ink)]">{t('impactPreview.title')}</h2></ScrollReveal>
          <div className="mt-14 grid overflow-hidden border border-black/10 bg-white/40 sm:grid-cols-2 lg:grid-cols-4">
            {metrics.map(([value, label], index) => <ScrollReveal key={label} delay={index * STAGGER} className="min-w-0"><motion.div whileHover={{ backgroundColor: 'rgba(255,255,255,.78)' }} transition={motionSpring.state} className="flex min-h-60 flex-col justify-between border-b border-r border-black/10 p-6 lg:p-7"><div className="flex items-center justify-between text-[9px] font-semibold tracking-[.18em] text-black/38"><span>0{index + 1}</span><span className="h-px w-6 bg-[var(--ss-bronze)]" /></div><div><p className="break-words text-[clamp(2.35rem,4.3vw,4.8rem)] font-medium leading-none tracking-[-.072em] tabular-nums">{value}</p><p className="mt-3 text-xs font-medium text-black/45">{label}</p></div></motion.div></ScrollReveal>)}
          </div>
        </div>
      </section>
      ...
      <section className="bg-white px-5 py-24 sm:px-8 lg:py-36">
        <div className="mx-auto max-w-[90rem]">
          <ScrollReveal className="flex flex-col gap-7 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-[10px] font-semibold uppercase tracking-[.28em] text-black/42">Selected work</p><h2 className="mt-5 text-[clamp(3rem,6vw,6rem)] font-medium leading-[.9] tracking-[-.068em]">{t('featured.title')}</h2></div><Link to="/projects" className="group inline-flex items-center gap-2 text-sm font-semibold">{t('featured.viewAll')}<ArrowUpRight className="size-4 transition-transform duration-500 group-hover:translate-x-1 group-hover:-translate-y-1" /></Link></ScrollReveal>
      ...
      <section className="relative overflow-hidden bg-[var(--ss-night)] px-5 py-24 text-white sm:px-8 lg:py-36">
        ...
        <div className="relative mx-auto max-w-[90rem]">
          <ScrollReveal><p className="text-[10px] font-semibold uppercase tracking-[.3em] text-white/38">Your move</p><h2 className="mt-6 max-w-5xl text-[clamp(3.5rem,8vw,9rem)] font-medium leading-[.84] tracking-[-.082em]">{t('cta.title')}</h2></ScrollReveal>
