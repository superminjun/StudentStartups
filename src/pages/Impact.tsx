...
function Metric({ metric, index }: { metric: ImpactMetricRecord; index: number }) {
  ...
  return (
    <motion.div ref={ref} initial={{ opacity: 0, y: 24, scale: .95 }} whileInView={{ opacity: 1, y: 0, scale: 1 }} viewport={{ once: true }} transition={{ ...motionSpring.reveal, delay: index * STAGGER }} whileHover={{ y: -8 }} className="flex min-h-64 flex-col justify-between border-b border-r border-black/10 p-6" style={{ background: colors[index % colors.length] }}>
      ...
      <div><p className="break-words text-[clamp(2.5rem,5vw,5.5rem)] font-semibold leading-none tracking-[-.08em] tabular-nums">{metric.prefix}{count.toLocaleString()}{metric.suffix}</p><p className="mt-3 text-sm font-semibold text-black/55">{lang === 'en' ? metric.labelEn : metric.labelKo}</p></div>
    </motion.div>
  );
}
...
function ChartCard({ no, title, body, color, children, wide = false }: { no: string; title: string; body: string; color: string; children: ReactNode; wide?: boolean }) {
  return (
    <ScrollReveal className={wide ? 'lg:col-span-2' : ''} amount={.18}>
      <motion.article whileHover={{ y: -7 }} transition={motionSpring.state} className="overflow-hidden rounded-[1.9rem] border border-white/10 p-3 shadow-[0_30px_80px_rgba(0,0,0,.25)]" style={{ background: color }}>
  ...
  return (
    <div className="bg-[var(--ss-canvas)] pt-[4.75rem]">
      <section className="relative overflow-hidden px-5 py-20 sm:px-8 lg:py-28">
        <motion.div animate={{ rotate: 360 }} transition={{ duration: 32, repeat: Infinity, ease: 'linear' }} className="absolute -right-24 -top-40 size-[34rem] rounded-full border border-black/10"><span className="absolute bottom-[12%] left-[12%] size-28 rounded-full bg-[var(--ss-lime)]" /></motion.div>
        <div className="relative mx-auto max-w-[90rem]">
          <motion.p initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={motionSpring.reveal} className="text-[10px] font-black uppercase tracking-[.28em] text-black/45">Record / Numbers with consequences</motion.p>
          <motion.h1 initial={{ opacity: 0, y: 46, rotateX: 5, filter: 'blur(9px)' }} animate={{ opacity: 1, y: 0, rotateX: 0, filter: 'blur(0px)' }} transition={motionSpring.depth} className="mt-8 text-[clamp(5rem,13vw,13rem)] font-semibold leading-[.72] tracking-[-.105em]">{t('impact.title')}</motion.h1>
          <motion.div initial={{ opacity: 0, y: 22 }} animate={{ opacity: 1, y: 0 }} transition={{ ...motionSpring.reveal, delay: .2 }} className="mt-14 grid gap-8 border-t border-black/20 pt-7 lg:grid-cols-[1fr_.65fr]"><p className="max-w-3xl text-[clamp(1.5rem,3vw,3.2rem)] font-medium leading-[1.08] tracking-[-.045em]">{lang === 'ko' ? '의도는 좋을 수 있습니다. 결과는 기록되어야 합니다.' : 'Intent can be good. Outcomes still need a record.'}</p><p className="max-w-lg text-base leading-7 text-black/55">{t('impact.subtitle')}</p></motion.div>
        </div>
      </section>
