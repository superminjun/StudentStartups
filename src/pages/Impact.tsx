...
function Metric({ metric, index }: { metric: ImpactMetricRecord; index: number }) {
  ...
  return (
    <motion.div ref={ref} initial={{ opacity: 0, y: 28, scale: .98 }} whileInView={{ opacity: 1, y: 0, scale: 1 }} viewport={{ once: true }} transition={{ ...motionSpring.reveal, delay: index * STAGGER }} whileHover={{ backgroundColor: 'rgba(255,255,255,.72)' }} className="flex min-h-64 flex-col justify-between border-b border-r border-black/10 bg-[var(--ss-panel)] p-6">
...
function ChartCard({ no, title, body, color, children, wide = false }: { no: string; title: string; body: string; color: string; children: ReactNode; wide?: boolean }) {
  return (
    <ScrollReveal className={wide ? 'lg:col-span-2' : ''} amount={.18}>
      <motion.article whileHover={{ y: -4 }} transition={motionSpring.state} className="overflow-hidden rounded-[1.5rem] border border-white/10 p-3 shadow-[0_30px_80px_rgba(0,0,0,.25)]" style={{ background: color }}>
